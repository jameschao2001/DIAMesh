...
**Mean normal deviation 是唯一可信 KPI**：face 越少 → normal 越粗略 → deviation 越大。完美線性、跟 face_count 一致、跟視覺感受一致。詳見 §6.8 Case Study 的 5AxisGlueSpraying.fbx 實測（12.06° / 6.95° / 1.61° 對應 TIER 1 / 2 / 3）。
...
# A-class advanced flags (all opt-in)
diamesh reduce <file.fbx> --backend blender --ratio 0.1 \
    --weld-tolerance-frac 5e-5         `# B3: adaptive weld (default)` \
    --fix-non-manifold                 `# B4: dissolve 3+ face edges` \
    --auto-fill-holes --fill-holes-smooth \
    --fill-holes-smooth-iter 2 --fill-holes-smooth-factor 0.5 \
    -o <out.fbx>                       `# C2: blend patches in`
...
**版本**：2026-05-03 — A 類 mesh repair 進深（B3+B4+C2+C1）
